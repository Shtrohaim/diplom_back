...
const scraperObject = {
    ...
    async scraper(browser){
        ...
        scrapedData.push( { 'pageTitle': 'ОГЭ', 'pageDescription' : description, 'url': 'https://fipi.ru/oge'} )

        let newPage = await browser.newPage();
        await newPage.goto('https://fipi.ru/oge/normativno-pravovye-dokumenty', {
            waitUntil: 'load',
            timeout: 0
        });
		...
		if(scrapedData.length !== 0){
			updateData(scrapedData, 'fipi_oge');
		}    
		await page.close();	
    }
}
